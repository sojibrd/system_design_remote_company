...
- **তিন repo-র কোড হুবহু এক** — পার্থক্য শুধু `app/lib/site.ts` (নাম, `lsd`/`rsd`/`gsd` prefix, `suggestedStart` — লোকালে `2026-11-02`, বাকি দুটোয় `null`), `next.config.ts`-এর basePath, `package.json`-এর নাম আর `docs/`; `guide/` আর simulator তিনটাতেই হুবহু। কোড বদলালে তিনটাতেই একই বদল।
...
- কাজে `(ডক ১৩)` / `(ডক ১১ · ডক ১৩)` = `guide/`-এর ডক — অবসরপ্রাপ্ত `system_design`-এর ২৫টা ডক হুবহু (২০২৬-০৯-১৬); ফাইল না থাকলে **build ভাঙে**। `(sim url-shortener · functional)` = এই সাইটের simulator (`app/lib/simulations/`, `/simulation/<id>/`)-এর সিস্টেম আর লেভেল; তালিকা `app/lib/sources.ts`-এ হাতে লেখা — না মিললে build ভাঙে, simulation-এ সিস্টেম বা লেভেল যোগ হলে ওখানেও। simulator-এর ভেতরের রেফারেন্স ধরে `npm run check:simulations` (CI-তে build-এর আগে)। `/docs/` আর `/doc/<nn>/` দেখায় কোন ডক কোন দিনে, তারপর পুরো ডক। `app/lib/guide.ts` server-only।
