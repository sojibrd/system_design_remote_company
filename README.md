...
- **Rail:** ৪টা পাতার লিংক, gauge আর ৪টা ব্লক। শুধু খোলা ব্লকের দিনগুলো দেখায়।
- **দিন (`/day/<nnn>/`) · ব্লক (`/block/<slug>/`):** দিনের কাজ আর দিন বা ব্লক শেষের হ্যাঁ/না। plan-এর হালকা দিনে এখানে কাজ নেই — কেন ফাঁকা তা লেখা।
- **সূত্রের chip:** কাজে `(ডক ১৩)` থাকলে `system_design` সাইটের ঐ ডকের লিংক; `(sim url-shortener · reliable)` থাকলে simulator-এর ঐ সিস্টেম।
...
- **design doc-এর লেখা এই repo-তে নয়** — `system_design/designs/` আর নিজের প্রজেক্টের repo-তে। সাইটে নোটের ঘর নেই, ইচ্ছাকৃত।
...
`legacy_and_wisdom/docs/ASSUMPTIONS.md` → `brainstorming/` (`system-design-*.md`, `crack-remote-company-roadmap.md`) → `switch_remote_company_in_6_month/docs/` → এই ফোল্ডার।
...
- কাজে `(ডক ১৩)` বা `(ডক ১১ · ডক ১৩)` = `system_design`-এর ডক; `(sim rate-limiter · reliable)` = simulator। তালিকা `app/lib/sources.ts`-এ।
